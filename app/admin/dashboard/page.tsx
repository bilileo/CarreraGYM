  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filtrados.map(p => {
        ...
        return [
          escape(getDisplayId(p.id)),
          escape(p.nombre),
          escape(p.correo),
          escape(p.edad),
          escape(p.categoria),
          escape(new Date(p.created_at).toLocaleDateString())
        ].join(',');
      })
    ].join('\n');
      ...
      <div id="admin-dashboard" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4">
            ...
            {/* BOTÓN DE EXPORTAR CSV AÑADIDO AQUÍ */}
            <button 
              onClick={exportToCSV} 
              className="bg-green-50 text-green-700 hover:bg-green-100 border border-green-200 px-4 py-3 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2"
            >
